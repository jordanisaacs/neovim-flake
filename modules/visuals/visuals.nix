  ...
  config = mkIf cfg.enable
    {
      vim.startPlugins = with pkgs.neovimPlugins; [
        ...
        (if cfg.lspkind.enable then pkgs.neovimPlugins.lspkind else null)
        (if cfg.cursorWordline.enable then nvim-cursorline else null)
        (if cfg.indentBlankline.enable then indent-blankline else null)
      ];

      vim.luaConfigRC = ''
        ${if cfg.lspkind.enable then "require'lspkind'.init()" else ""}
        ${if cfg.indentBlankline.enable then ''
          -- highlight error: https://github.com/lukas-reineke/indent-blankline.nvim/issues/59
          vim.wo.colorcolumn = "99999"
          vim.opt.list = true


          ${if cfg.indentBlankline.eolChar == "" then
            ""
            else
            ''vim.opt.listchars:append({ eol = "${cfg.indentBlankline.eolChar}" })'' }

          ${if cfg.indentBlankline.fillChar == "" then
            ""
            else
        ...
        ${if cfg.cursorWordline.enable then "vim.g.cursorline_timeout = ${toString cfg.cursorWordline.lineTimeout}" else ""}
      '';
    };
}
