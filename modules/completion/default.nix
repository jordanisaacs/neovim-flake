...
in {
  ...
  config = mkIf (cfg.enable) (
    ...
    in {
      vim.startPlugins = with pkgs.neovimPlugins; (if cfg.type == "nvim-compe" then [
        nvim-compe
      ] else []) ++ (if cfg.type == "nvim-cmp" then [
        nvim-cmp
        cmp-buffer
        cmp-vsnip
        cmp-path
          ...
          cmp.setup({
            ...
            formatting = {
              format = function(entry, vim_item)
                -- type of kind
                vim_item.kind = ${writeIf (config.vim.icons.lspkind) "require('lspkind').presets.default[vim_item.kind] .. ' ' .."} vim_item.kind
      ...
      vim.snippets.vsnip.enable = if (cfg.type == "nvim-cmp") then true else config.vim.snippets.vsnip.enable;
    }
  );
}
